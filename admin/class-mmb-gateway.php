...
class Moneta extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->id = 'moneta';
        $this->method_title = __('Moneta', 'mmb-gateway-woocommerce');
        $this->method_description = __('Moneta gateway sends customers to Moneta to enter their payment information and redirects back to shop when the payment was completed.', 'mmb-gateway-woocommerce');
...
//         $this->api_available_payment_solutions = $this->get_option('api_available_payment_solutions');
        ...
        $api_test_token_url = 'https://apiuat.test.monetaplatebnisluzby.cz/token';
        $api_test_payments_url = 'https://apiuat.test.monetaplatebnisluzby.cz/payments';
        $api_test_js_url = 'https://cashierui-apiuat.test.monetaplatebnisluzby.cz/js/api.js';
        $api_test_cashier_url = 'https://cashierui-apiuat.test.monetaplatebnisluzby.cz/ui/cashier';
        $api_token_url = 'https://api.monetaplatebnisluzby.cz/token';
        $api_payments_url = 'https://api.monetaplatebnisluzby.cz/payments';
        $api_js_url = 'https://cashierui-api.monetaplatebnisluzby.cz/js/api.js';
        $api_cashier_url = 'https://cashierui-api.monetaplatebnisluzby.cz/ui/cashier';
    ...
    /**
     * Check if the currency of store is accepted by Moneta
     *
     * TODO: ez kell?
     *
     * @since    1.0.0
    ...
    public function is_valid_for_use() {
        if (!in_array(get_woocommerce_currency(),
            ...
            $this->msg = sprintf(__("Moneta doesn't accept your store currency. Check available currencies  %s here", 'mmb-gateway-woocommerce') . "</a>");
            return false;
        }
...
//             }
    ...
    public function capture_payment( $order_id ) {
        $order = wc_get_order( $order_id );
        if ( $order->get_payment_method() == 'moneta' ) {
    ...
    public function add_capture_charge_order_action( $actions ) {
        ...
        if ( 'moneta' !== $payment_method || 'on-hold' !== $payment_status ) {
            return $actions;
        }
        ...
        $actions['moneta_capture_charge'] = esc_html__( 'Capture Online');
    ...
    public function add_void_charge_order_action( $actions ) {
        ...
        $actions['moneta_void_charge'] = esc_html__( 'VOID Online');
