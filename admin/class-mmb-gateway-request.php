...
class MMB_Gateway_Request
{
    ...
    public function __construct($gateway)
    {
        ...
        $this->notify_url = WC()->api_request_url('Moneta');
    ...
    private function get_mmb_gateway_token_args($order, $sandbox = false)
    {
        ...
        return array(
            ...
            "merchantLandingPageUrl" =>  $shop_page_url . "?wcapi=moneta&order_id=" . $order->get_id(),
...
//             customerAddressPhone($post_data['customerAddressPhone'])->
        ...
        switch($this->gateway->api_payment_modes){
            ...
            case '0':
                ...
                $mmb_form[] = '" method="get"><input type="hidden" name="wcapi" value="moneta"/><input type="hidden" name="order_id" value="'.$order->get_id().'"/><input type="hidden" id="merchantTxId" name="merchantTxId" value=""/></form>';
    ...
    private function get_mmb_gateway_redirect_mode($sandbox = false,$token){
        ...
        $form_html .= '<button type="submit" class="button alt">'.__( 'Pay with Moneta', 'mmb-gateway-woocommerce' ).'</button> </form>';
        return $form_html;
    }
    /**
